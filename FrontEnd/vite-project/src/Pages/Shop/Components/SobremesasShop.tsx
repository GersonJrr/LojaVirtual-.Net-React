   ...
   useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        const categoriaPizza = data.find((cat: any) => cat.categoriaId === 7);
    ...
    <div className="px-4 py-8">
      <div className="flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-40 gap-6">
          {produtos.slice(0, limite).map((sobremesas) => (
            <CardProduto
              key={sobremesas.id}
              id={sobremesas.id}
              nome={sobremesas.nome}
              preco={sobremesas.preco}
              descricao={sobremesas.descricao}
              imageURL={sobremesas.imageURL}
            />
          ))}
        </div>
      </div>
